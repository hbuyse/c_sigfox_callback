                       ...
                       )
{
    if ( f->do_realloc && (f->num_tokens >= f->max_tokens) )
    {
        ...
        if ( p == NULL )
        {
            return (JSON_TOKEN_ARRAY_TOO_SMALL);
        }
    ...
    if ( (f->tokens == NULL) || (f->max_tokens == 0) )
    {
        return (0);
    }

    if ( f->num_tokens >= f->max_tokens )
    {
        return (JSON_TOKEN_ARRAY_TOO_SMALL);
    }
                                        ...
                                        )
{
    while ( path != 0 && path[0] != '\0' )
    {
        ...
        if ( path[0] == '[' )
        {
            if ( (toks->type != JSON_TYPE_ARRAY) || ! is_digit(path[1]) )
            {
                return (0);
            }
        ...
        for ( i = 0; i < toks[-1].num_desc; i += skip, ind2++ )
        {
            ...
            if ( (ind == -1) && (toks[i].type != JSON_TYPE_STRING) )
            {
                return (0);
            }
...
static enum v7_err     mg_send_js(struct v7 *v7,
                                  v7_val_t  *res
                                  )
{
...
enum v7_err     mg_enable_javascript(struct mg_mgr  *m,
                                     struct v7      *v7,
...
static int mg_use_ca_cert(SSL_CTX       *ctx,
                          const char    *cert
                          )
{
    if ( ctx == NULL )
...
static int mg_use_cert(SSL_CTX      *ctx,
                       const char   *cert,
                       const char   *key
                       )
{
 ...
 */
static const char* mg_set_ssl2(struct mg_connection *nc,
                               const char           *cert,
                               const char           *key,
                               const char           *ca_cert
                               )
                       ...
                       )
{
    ...
    #else
const char* mg_set_ssl2(struct mg_connection *nc, const char *cert, const char *key, const char *ca_cert);
...
static void mg_handle_udp_read(struct mg_connection *nc)
{
    ...
    #if defined(MG_ENABLE_SSL)
static int mg_ssl_err(struct mg_connection  *conn,
                      int                   res
                      )
{
...
static void link_conns(struct mg_connection *c1,
...
static void forwarder_ev_handler(struct mg_connection   *c,
                                 int                    ev,
                                 void                   *p
                                 )
{
...
static void multithreaded_ev_handler(struct mg_connection   *c,
                                     int                    ev,
                                     void                   *p
                                     )
{
                                 ...
                                 )
{
    ...
    switch ( ev )
    {
        ...
        #ifndef MG_EXT_SHA1
static void mg_hash_sha1_v(size_t           num_msgs,
                           const uint8_t    *msgs[],
...
static void mg_http_handler2(struct mg_connection   *nc,
                             int                    ev,
                             void                   *ev_data,
                             struct http_message    *hm
                             )
...
static void mg_http_multipart_call_handler(struct mg_connection *c,
                                           int                  ev,
                                           const char           *data,
                                           size_t               data_len
                                           )
 ...
 */
static void mg_handle_lock(struct mg_connection *nc,
                           const char           *path
                           )
{
...
static int mg_wait_until_ready(sock_t   sock,
                               int      for_read
                               )
{
    fd_set     set;
...
static void mg_spawn_stdio_thread(sock_t    sock,
                                  HANDLE    hPipe,
                                  void *(*func)(void *)
                                  )
{
...
static void mg_abs_path(const char  *utf8_path,
                        char        *abs_path,
                        size_t      len
                        )
{
...
#ifdef MG_ENABLE_THREADS
void* mg_start_thread(void *(*f)(void *),
                      void *p
                      )
{
    #ifdef _WIN32
...
void mg_mqtt_broker(struct mg_connection    *nc,
                    int                     ev,
                    void                    *data
                    )
{
 ...
 * Helper function.
 */
static char* coap_parse_header(char                     *ptr,
                               struct mbuf              *io,
 ...
 * Helper function.
 */
static char* coap_get_token(char                    *ptr,
                            struct mbuf             *io,
                            struct mg_coap_message  *cm
                            )
{
 ...
 * Helper function.
 */
static int coap_get_ext_opt(char        *ptr,
                            struct mbuf *io,
                            uint16_t    *opt_info
                            )
{
 ...
 * +-------------------------------+
 */
static char* coap_get_options(char                      *ptr,
                              struct mbuf               *io,
                              struct mg_coap_message    *cm
                              )
{
 ...
 * Helper function.
 */
static int coap_split_opt(uint32_t  value,
                          uint8_t   *base,
                          uint16_t  *ext
                          )
{
 ...
 * Helper function.
 */
static char* coap_add_uint16(char       *ptr,
                             uint16_t   val
                             )
{
    *ptr    = val >> 8;
 ...
 * Helper function.
 */
static char* coap_add_opt_info(char     *ptr,
                               uint16_t val,
                               size_t   len
                               )
{
...
static void coap_handler(struct mg_connection   *nc,
                         int                    ev,
                         void                   *ev_data
                         )
{
...
int fs_slfs_open(const char *pathname,
                 int        flags,
                 mode_t     mode
                 )
{
...
int fs_slfs_stat(const char     *pathname,
                 struct stat    *s
                 )
{
    SlFsFileInfo_t      sl_fi;
...
int fs_slfs_rename(const char   *from,
                   const char   *to
                   )
{
    return (set_errno(ENOTSUP) );
...
void fs_slfs_set_new_file_size(const char   *name,
                               size_t       size
                               )
{
    int     i;
...
void mg_run_in_task(void (*cb)(struct mg_mgr *mgr, void *arg),
                    void *cb_arg
                    )
{
...
static sock_t mg_open_listening_socket(union socket_address *sa, int type, int proto);
    ...
    #ifdef MG_ENABLE_SSL
const char* mg_set_ssl2(struct mg_connection    *nc,
                        const char              *cert,
                        const char              *key,
                        const char              *ca_cert
                        )
